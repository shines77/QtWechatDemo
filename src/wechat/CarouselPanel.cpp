...
CarouselPanel::CarouselPanel(QWidget *parent) : QWidget(parent)
{
...
void CarouselPanel::SetImageList(QStringList listImgFileName)
{
    m_listImgFileName = listImgFileName;
}

void CarouselPanel::StartPlay()
{
    UpdateChangeBtns();
...
void CarouselPanel::CreateAllCtrls()
{
    m_pTimerImageChange = new QTimer(this);
}

void CarouselPanel::InitCtrls()
{
    // 添加ImageOpacity属性;
...
void CarouselPanel::InitSlots()
{
    connect(m_pTimerImageChange, SIGNAL(timeout()), this, SLOT(OnImageChangeTimeout()));
}

void CarouselPanel::Relayout()
{
    //
}

void CarouselPanel::UpdateChangeBtns()
{
    ...
    for (int i = 0; i < m_listImgFileName.count(); i++)
    {
        ...
        pButton->setProperty("CarouselPanel", "true");
...
void CarouselPanel::paintEvent(QPaintEvent *event)
{
...
void CarouselPanel::OnImageChangeTimeout()
{
    m_pixCurrent = QPixmap(m_listImgFileName.at(m_nCurrentImageIndex));
...
void CarouselPanel::OnImageSwitchButtonClicked(int nIndex)
{
